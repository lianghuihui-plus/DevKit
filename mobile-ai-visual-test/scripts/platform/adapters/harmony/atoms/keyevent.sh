...
output="$("${hdc_prefix[@]}" shell uitest uiInput keyEvent "$key" 2>&1)" || { printf '%s\n' "$output" >&2; exit 1; }
...
console.log(JSON.stringify({schemaVersion:1,type:"actionResult",platform:"harmony",time:localIso(),action:"keyevent",key:process.argv[1],ok:true}, null, 2));
' "$key"
