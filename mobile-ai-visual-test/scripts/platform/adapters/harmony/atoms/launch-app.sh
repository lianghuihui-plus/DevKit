...
[[ -n "$bundle" && -n "$ability" ]] || { echo "launch-app 需要 --bundle/--app 和 --ability/--entry" >&2; exit 2; }
...
output="$("${hdc_prefix[@]}" shell aa start -b "$bundle" -a "$ability" 2>&1)" || { printf '%s\n' "$output" >&2; exit 1; }
...
console.log(JSON.stringify({schemaVersion:1,type:"actionResult",platform:"harmony",time:localIso(),action:"launchApp",ok:true}, null, 2));
'
