...
| 文件 | 职责 |
|------|------|
| `test-plans/*.yaml` | 测试意图和步骤；v2 使用结构化 action / assert |
...
列出 `test-plans/` 下的 YAML，用户选择要生成脚本的。
...
1. 解析 YAML → `schemaVersion`, `caseId`, `target`, `source`, `sourceHash`, `navigation`, `precondition`, `steps`, `assert`
...
- `schemaVersion: 2` 的 YAML 必须包含 `caseId`、`source` 和 `sourceHash`，并且每个 step 必须有 `action.kind`。
- `schemaVersion: 2` 的 YAML 必须同步校验 `sourceHash`；源文件不存在或 hash 不一致时，提示"源用例已变化，建议先重新执行 case-gen"，用户确认继续时仍可生成脚本，但不得读取旧 success-path 作为证据源。
- 缺少 `schemaVersion` 的旧 YAML 允许字符串 action / assert；生成时先转换为等价的内部结构，再翻译 ArkTS。
...
- 生成代码不得包含 `any`、`Object`、匿名对象字面量、动态索引、`Level` 或 `done` 回调。

---

### Step 4：定位目标模块

读取 `CONTEXT.md` 的 `## 项目` 获取代码仓库路径，从 `## 路由表` 找到 `pageSourceFile`，反推模块路径：
...
旧 YAML 字符串 action / assert 先按下表兼容转换；无法转换时保留自然语言注释并生成保守 UI 断言。
...
同一 YAML 重复生成时，v2 YAML 优先根据 `caseId` 定位已有 `it()` 并更新该用例；旧 YAML 再根据 `description` 或稳定用例名定位。无法确认对应关系时追加新的 `it()`，并在展示代码时说明。
