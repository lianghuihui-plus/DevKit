...
| 文件 | 职责 |
|------|------|
| `CONTEXT.md` | 工作空间全局信息：项目元数据、路由表、跨页面导航、全局设备特性和已知问题 |
...
| 路由 | 扫描 `**/route_map.json` → `name` → `pageSourceFile`（通常位于 `product/<product>/src/main/resources/base/profile/route_map.json`） |

> 如果标准路径无法提取完整元数据，回退搜索 `router.pushUrl`、`Navigation`、`UriRouterConstants`、`module.json5` 和路由常量定义；仍无法确认时，展示候选项让用户确认，不直接编造。
...
1. 创建目录结构
2. 复制 `templates/device-setup.md` 到 `references/device-setup.md`
3. 复制 `templates/AGENT.md` 到工作空间根目录
4. 从 `templates/` 拷贝官方文档 Markdown 到 `references/`（重命名为简短名）：
   - `@ohos.UiTest-ArkTS API-*.md` → `references/UiTest-API.md`
   - `UI测试框架使用指导-*.md` → `references/UiTest-指南.md`
5. 复制 `templates/test-source-template.md` 到 `test-sources/example.md`
...
| 页面名 | 源文件 |
|--------|--------|
| loginPage | product/lunar/src/main/ets/pages/Login.ets |
| ... | ... |
...
| 必需加载 | `CONTEXT.md` | 项目元数据、路由表、页面导航、全局记录 | 必须读取；不存在则提示重新执行 session 新建 |
...
| 摘要汇总 | `test-plans/*.yaml` | 用例数量、caseId、target、schemaVersion | 只读索引和关键字段 |
