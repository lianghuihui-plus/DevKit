...
定位成功后，确认当前 workspace 已由 session 创建或恢复。后续设备检查、hdc 执行、自动修复、records 更新和报告写入必须遵守 `AGENT.md` 约束；特别是"遇阻先读代码"、"超限即停"、success-path 复用边界和 test-records 写入规则。
...
解析 YAML，提取 `schemaVersion`、`caseId`、`target`、`source`、`sourceHash`、`navigation`、`precondition`、`steps`。同时计算当前 YAML 原始字节 `sha256`，用于 success-path 和 `test-records.json`。

**版本判定**：
- `schemaVersion: 2` → 按 v2 结构化意图解析。
- 缺少 `schemaVersion` → 按旧 YAML 兼容解析，允许字符串 action / assert。
- 其他 `schemaVersion` → 停止该 YAML，记录为 `caseFailure`，提示当前 runner 不支持该版本。

**执行前校验**：
- YAML 必须包含 `target`、非空 `navigation` 和非空 `steps`。
- v2 YAML 必须包含非空 `caseId`、`source` 和 `sourceHash`；旧 YAML 可缺失。
- v2 YAML 必须同步校验 `sourceHash`；源文件不存在或 hash 不一致时，提示"源用例已变化"，但用户确认后仍可常规执行，不复用 success-path。
- v2 每个 step 必须有非空 `action.kind`；旧 YAML 每个 step 必须有非空字符串 `action`。
- v2 `assert` 必须是对象或对象数组；旧 YAML `assert` 允许字符串或字符串数组。
- 找不到 exploration 时允许继续实时探索，但报告中需记录本次缺少页面经验缓存。

**records 主键**：
- 若 YAML 有 `caseId`，用 `caseId` 作为 `test-records.json.cases` 主键。
- 若 YAML 没有 `caseId`（旧 YAML），使用 YAML 相对路径作为临时主键；后续迁移为新 YAML 后再改用稳定 `caseId`。
...
- `test-records.json`：读取当前用例的 `status`、`yamlHash`、`passStreak`、`successPath`、`lastFailure`；hash 不匹配时标记 stale 并禁用 fast path。
...
**records / hash 决策**：
- `yamlHash` 变化 → 禁用 success-path，标记 `stale`，走常规智能执行。
- `sourceHash` 变化 → 提示源用例已变化，建议重新执行 case-gen；用户确认继续时禁用 success-path，走常规智能执行。
- `explorationHash` 变化 → 禁用 fast path 一次，常规执行通过后刷新 records 和 success-path。
- Bundle / Product 变化 → 禁用 success-path，重新确认环境。
- Device 变化 → 可降级复用，但必须更严格校验页面锚点和目标控件。
- 旧 YAML 没有 `caseId` 时，records 只做运行状态记录，不强制迁移 YAML。
...
- 无法确认 App 前台状态、`fixedStartAnchors` 或当前页面 dumpLayout 时，不尝试 success-path，直接进入常规智能执行或汇报设备卡点。
...
- fast path 中途失败时，不继续硬套后续缓存记录，从固定起点重新进入常规智能执行流程；如无法回到固定起点，汇报用户。
...
- 导航失败 → 按规则 A 读代码换策略，最多 3 次；仍失败按规则 B 汇报。
...
- 修正代价大 → 汇报用户
- 修正最多 3 次 → 仍失败按规则 B 汇报

#### 4.7 逐 step 执行

**action 定义**：action 是**逻辑操作**。执行时拆为物理操作。v2 YAML 使用结构化 action；旧 YAML 使用自然语言 action，并按当前 NLP 规则解析。
...
**成功命令采集**：无论 fast path 还是常规执行，只要本次物理命令最终成功，且能生成可校验的 `before` 和 `after` 证据，就先写入内存中的候选成功路径；只有整个 YAML 通过后，才落盘覆盖对应 `success-paths/<yaml-basename>.jsonl`。无法生成有效状态指纹的命令只写入测试报告，不进入 success-path。

每个逻辑操作执行完后，UI 必须回到干净状态（无键盘、无弹窗）。

每次操作的标准流程：
```
解析 action → dumpLayout → 结合 exploration 页面经验和历史 layoutTree 证据 → 找目标控件
→ uiInput click/inputText → 等页面稳定
→ 有 assert？→ dumpLayout → 匹配断言规则 → 通过/失败
→ 失败？→ 读代码分析原因 → 换策略重试（最多 3 次）→ 仍失败按规则 B 汇报
```
...
旧 YAML 字符串断言按原自然语言规则解析；能转成上述结构化断言时按结构化方式执行，不能转时按 `naturalLanguage` 处理。

**弹窗**：每次 dumpLayout 后检查是否有弹窗，存在则先关闭再继续。弹窗处理参考 exploration 的"弹窗与遮挡"和"hdc 探索记录"。

**异常**：hdc 命令失败（设备断开、App 崩溃等）→ 立即暂停，汇报用户。
...
| `fatalDeviceIssue` | 设备断开、App 无法启动、连续无法 dumpLayout | 暂停批量流水线，记录当前位置，等待用户处理 |

#### 4.8 文件完成

- **通过** → 写入 / 刷新成功路径缓存，更新 `test-records.json` 为 `verified` 或 `trusted`，建议结果：`✅ xxx.yaml 通过`
...
- `fatalDeviceIssue`：暂停批量流水线，不继续污染后续用例记录。
...
字段要求：
- 第一行 `type="meta"` 必须记录 `yaml`、`yamlHash`、`target`、`bundle`、`product`、`device`、`fixedStartAnchors`、`createdAt`；v2 YAML 还必须记录 `caseId`、`source`、`sourceHash`、`explorationHash`。
...
| 用例 | 使用情况 | 写入状态 | 文件 | 说明 |
|------|----------|----------|------|------|
| accountLoginPage-1.yaml | 已使用 | 已刷新 | explorations/accountLoginPage/success-paths/accountLoginPage-1.jsonl | fast path 校验通过，按实时 dumpLayout 重新生成命令 |
| accountLoginPage-2.yaml | 失效回退 | 未覆盖 | explorations/accountLoginPage/success-paths/accountLoginPage-2.jsonl | YAML hash 不一致 |
...
**更新 success-paths**：仅当单个 YAML 完整通过时覆盖写入对应成功路径。失败、跳过、用户终止、设备异常时不得覆盖旧成功路径；报告中记录旧缓存是否被使用、是否失效、失效原因。

如果对应页面的 `exploration.md` 不存在，按 case-gen 的标准章节创建，至少包含：`## 导航`、`## 控件定位`、`## 状态规律`、`## 弹窗与遮挡`、`## 代码分析`、`## hdc 探索记录`、`## 已验证修正`、`## 失败尝试记录`。其中 `## 控件定位`、`## 已验证修正`、`## 失败尝试记录` 必须创建结构化表头：
...
- 使用 `caseId` 作为 records 主键；旧 YAML 无 `caseId` 时临时使用 YAML 相对路径。
- 成功：写入 `yaml`、`target`、`yamlHash`、`source`、`sourceHash`、`explorationHash`、`layoutTreeHash`、`pageFingerprint`、`successPath`，`runCount + 1`、`passStreak + 1`；普通用例 `passStreak >= 3` 时 status 为 `trusted`，否则为 `verified`。
- smoke 用例（`caseId` 以 `smoke-` 开头，或文件名为 `smoke-<page>.yaml`）通过后 status 最高只写 `verified`，不升级 `trusted`；它只证明页面可达和核心锚点存在，不代表业务用例可信。
- 失败：`runCount + 1`、`passStreak = 0`；按失败分类写 `status`、`lastFailure` 和 `lastAttempt.result`。
