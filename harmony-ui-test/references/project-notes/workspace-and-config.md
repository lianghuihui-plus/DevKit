...
默认优先把命令、结果摘要、关键输出片段和步骤复现结论写入 case report，不额外创建日志文件。

只有以下情况才写入 `logs/`：

- 原始输出过长，放入 report 会影响阅读。
- 失败定位必须保留原始证据，例如步骤复现输出、控件树、截图、关键 hilog 片段或构建失败长日志。
...
该文件可包含 runner 关键原始输出、步骤复现命令、关键片段和附件路径。不要为同一个 case 的每条命令都创建独立 log 文件。
