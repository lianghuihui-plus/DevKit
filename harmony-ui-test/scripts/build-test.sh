...
usage() {
  cat <<'EOF'
Usage:
  build-test.sh --deveco-sdk-home PATH --node PATH --hvigor PATH --product NAME --module NAME [options]

Options:
  --project-root PATH       HarmonyOS project root. Defaults to current directory.
  --build-mode MODE         Test build mode. Defaults to test.
  --task NAME               Hvigor test build task. Defaults to genOnDeviceTestHap.
  --module-target NAME      Full module target. Overrides --module suffix logic.
  --hvigor-flag FLAG        Extra safe hvigor flag. Repeatable. Allowed: --no-daemon.
  --no-ohos-test-suffix     Use --module value as-is instead of appending @ohosTest.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --module) [[ $# -ge 2 ]] || die "--module requires a value"; module_name="$2"; shift 2 ;;
    --module-target) [[ $# -ge 2 ]] || die "--module-target requires a value"; module_target="$2"; shift 2 ;;
    ...
    --hvigor-flag)
      ...
      shift 2
      ;;
    --no-ohos-test-suffix) append_ohos_test=0; shift ;;
...
[[ -n "$module_name" || -n "$module_target" ]] || die "--module or --module-target is required"
...
if [[ -z "$module_target" ]]; then
  ...
  if [[ "$append_ohos_test" -eq 1 && "$module_target" != *"@"* ]]; then
    module_target="${module_target}@ohosTest"
  fi
fi
