...
- **记录职责清晰**：`CONTEXT.md` 只记录全局项目、路由索引、已解析页面路由、跨页面导航和全局问题；页面级控件定位、状态规律、弹窗处理、页面内陷阱、已验证修正和失败尝试记录写入 `explorations/<page>/exploration.md`
...
- **YAML 使用结构化字段**：case-gen 新生成 YAML 必须包含稳定唯一 `caseId`、`source`、`sourceHash` 和结构化 `action.kind` / `assert.type`；历史字符串写法仅作为兼容执行
...
- **success-path 是通过门禁**：runner 执行每个 YAML 时必须初始化 `successPathBuffer`，采集 navigation / precondition / step / assert / cleanup 的 before/after 证据；YAML 通过时必须写入 success-path，或在 report 中明确说明不能写入的原因，禁止静默跳过
...
- **长流程少打断**：case-gen / runner 启动后，单个源文件、页面或 YAML 的信息不足、定位失败、断言失败等异常应写入对应产物并继续后续项，不频繁中途询问用户
- **超限记录**：同一目标重试 3 次无果 → 当前源文件 / 页面 / YAML 记录为需人工处理、failing 或 blocked；只有设备断开、App 无法启动、连续无法 dumpLayout 等 fatal 设备问题才暂停批量流水线
...
- 禁止动态属性索引访问（`obj[key]`、`delete obj[key]`）
- 禁止 `Partial<>` 等工具类型
- 类型窄化不穿透 `forEach` 闭包
