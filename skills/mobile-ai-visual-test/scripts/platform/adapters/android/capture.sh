...
args=(--out "$png"); [[ -n "$device" ]] && args=(--device "$device" "${args[@]}")
...
console.log(JSON.stringify({schemaVersion:1,type:"capture",platform:"android",time:ended,artifacts:{screenshot:ref},device:{id:device||null},app:{appId:app},captureTiming:{order:["screenshot"],captureStartedAt:started,screenshotCompletedAt:ended,spanMs:Math.max(0,Date.parse(ended)-Date.parse(started))}},null,2));
