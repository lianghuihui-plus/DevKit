...
console.log(JSON.stringify({schemaVersion:1,type:"actionResult",platform:"harmony",time:localIso(),action:"longPress",ok:true,durationMs:Number(process.argv[1]),executedPoint:{x:Number(process.argv[2]),y:Number(process.argv[3])}}, null, 2));
' "$duration_ms" "$x" "$y"
