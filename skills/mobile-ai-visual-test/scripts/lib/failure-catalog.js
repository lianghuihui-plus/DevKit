...
const FAILURE_CATALOG = Object.freeze({
  ...
  ACTION_COORDINATE_CONFLICT: { status: 'BLOCKED', label: '当前现场坐标空间存在冲突' },
  CASE_TIME_LIMIT_REACHED: { status: 'BLOCKED', label: '已达到单用例时限' },
  CASE_TIME_LIMIT_INSUFFICIENT: { status: 'BLOCKED', label: '剩余时间不足以完成动作和观察' },
  DEVICE_ACTION_FAILED: { status: 'BLOCKED', label: '设备动作执行失败' },
  DEVICE_ACTION_OUTCOME_UNCERTAIN: { status: 'BLOCKED', label: '设备动作结果不确定' },
  DEVICE_ADAPTER_FAILED: { status: 'BLOCKED', label: '设备适配器执行失败' },
  DEVICE_ADAPTER_OUTPUT_INVALID: { status: 'BLOCKED', label: '设备适配器输出无效' },
  DEVICE_OBSERVATION_OUTCOME_UNAVAILABLE: { status: 'BLOCKED', label: '观察事务无法在时限后恢复' },
  IOS_ACTION_FAILED: { status: 'BLOCKED', label: 'iOS 动作执行失败' },
  IOS_AUTOMATION_RELEASE_INCOMPLETE: { status: 'BLOCKED', label: 'iOS 自动化资源释放不完整' },
  IOS_INPUT_TEXT_FAILED: { status: 'BLOCKED', label: 'iOS 整串文本输入失败' },
  IOS_WDA_OWNERSHIP_LOST: { status: 'BLOCKED', label: 'iOS WDA 所有权记录不一致' },
  IOS_WDA_STOP_FAILED: { status: 'BLOCKED', label: 'iOS WDA 服务停止失败' },
  CURRENT_OBSERVATION_REQUIRED: { status: 'BLOCKED', label: '缺少最新现场证据' },
  EVIDENCE_CONFLICT_UNRESOLVED: { status: 'BLOCKED', label: '关键现场证据冲突尚未解决' },
