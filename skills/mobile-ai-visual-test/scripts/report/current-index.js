...
function summarize(cases) {
  ...
  return {
    ...
    passWithWaivers: cases.filter((item) => (item.platforms || []).some((platform) => platform.checkpointMetrics?.passWithWaivers)).length,
...
function renderStatus(value, checkpointMetrics = null) {
  const waiverSuffix = value === 'PASS' && checkpointMetrics?.waived ? ` · 含 ${checkpointMetrics.waived} 个豁免` : '';
  return `<span class="status ${escapeHtml(className(value))}${waiverSuffix ? ' pass-with-waivers' : ''}">${escapeHtml(verdictLabel(value))}${escapeHtml(waiverSuffix)}</span>`;
}
...
function renderPlatformRun(platform) {
  ...
  return `<article class="platform-run ${escapeHtml(platform.platform)}" data-platform-run="${escapeHtml(platform.platform)}">
    ...
    ${renderStatus(verdict, platform.checkpointMetrics)}
...
function caseFilterVerdicts(item) {
  ...
  const projected = verdicts.length ? verdicts : [dashboardVerdict(item)];
  if ((item.platforms || []).some((platform) => platform.checkpointMetrics?.passWithWaivers)) projected.push('PASS_WITH_WAIVERS');
...
function caseFilterResults(platforms) {
  return platforms.filter((platform) => PLATFORM_ORDER.includes(platform.platform)).flatMap((platform) => {
    const values = [`${platform.platform}:${dashboardVerdict(platform)}`];
    if (platform.checkpointMetrics?.passWithWaivers) values.push(`${platform.platform}:PASS_WITH_WAIVERS`);
    return values;
  });
}
...
function renderCase(item, index) {
  ...
  const reportIssues = Array.isArray(item.reportIssues) ? item.reportIssues : [];
  const reportHealth = item.reportHealth || (reportIssues.length ? 'DEGRADED' : 'HEALTHY');
  const issuePlatforms = reportIssues.map((issue) => displayPlatform(issue.platform)).join('、');
  const healthWarning = reportHealth === 'HEALTHY' ? ''
    : `<span class="report-health-warning" title="${escapeHtml(reportIssues.map((issue) => issue.reason).filter(Boolean).join('；'))}">报告健康异常${issuePlatforms ? ` · ${escapeHtml(issuePlatforms)}` : ''}</span>`;
  return `<section class="case-row" data-case-status="${escapeHtml(statusValues)}" data-case-platforms="${escapeHtml(platformValues)}" data-case-results="${escapeHtml(resultValues)}" data-report-health="${escapeHtml(reportHealth)}" data-case-search="${escapeHtml(`${item.caseNo || ''} ${item.title} ${item.caseKey || ''}`.toLowerCase())}">
    <div class="case-common">
      <div class="case-order">${String(index + 1).padStart(2, '0')}</div>
      <div class="case-copy"><span>用例 ${escapeHtml(item.caseNo || String(index + 1).padStart(3, '0'))} · ${escapeHtml(item.caseKey || '-')}</span>${healthWarning}<h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(summary)}</p></div>
...
function renderCurrentIndexHtml(rootDir, cases = []) {
  ...
  const filters = [
    ...
    ['PASS_WITH_WAIVERS', '含豁免', filterCount('PASS_WITH_WAIVERS')],
