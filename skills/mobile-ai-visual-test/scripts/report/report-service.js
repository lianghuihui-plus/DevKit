...
// Report orchestration and read-only compatibility for historical executions.
...
} = require('../lib/historical-case-contract');
...
function workspaceRoot(cwd = process.cwd()) {
  return assertWorkspace(cwd).root;
}
...
function readCaseRuntimeSummary(caseDir, caseJson, platform = '') {
  ...
  const latest = selectExecutionDir(runtimeDir)?.execDir || null;
  const published = readExecutionReport(latest);
  const result = published.result;
  const metrics = published.metrics;
  const display = published.display;
  const displayStatusValue = display?.status === 'INCONCLUSIVE' ? 'UNKNOWN' : display?.status;
  const status = displayStatusValue || result?.status || state.latestStatus || 'NOT_RUN';
  ...
  return {
    ...
    startedAt: display?.startedAt || result?.startedAt || '',
    endedAt: display?.endedAt || result?.endedAt || '',
    durationMs: display?.durationMs ?? metrics?.durationMs,
    stepsSummary: display?.stepsSummary || (steps ? `${steps.passed || 0}/${steps.total || 0}` : `${caseJson.steps?.length || 0}`),
    failureCode: display?.failureCode || result?.failureCode || state.latestFailureCode || '',
    failedStep: display?.failedStep || result?.failedStep || '',
    verdict: display?.verdict || null,
    verdictBasis: display?.verdictBasis || null,
    executionStatus: display?.executionStatus || null,
    currentMetrics: published.schemaFamily === 'current' ? metrics : null,
    schemaFamily: published.schemaFamily,
    reason: display?.summary || result?.reason || state.latestReason || '',
    updatedAt: display?.endedAt || result?.endedAt || state.environmentConfirmedAt || caseJson.identity?.sourceUpdatedAt || '',
  ...
  return fs.readdirSync(casesRoot)
    ...
    .map((caseDir) => {
      ...
      return {
        ...
        verdictBasis: primary.verdictBasis,
        executionStatus: primary.executionStatus,
        currentMetrics: primary.currentMetrics,
        schemaFamily: primary.schemaFamily,
...
function aggregateStatus(statuses = []) {
  ...
  if (statuses.includes('FINALIZATION_RECOVERY_REQUIRED')) return 'RUNNING';
  if (statuses.includes('PENDING_PUBLICATION')) return 'PENDING_PUBLICATION';
...
function assertIndexReportLinks(rootDir, cases) {
  const root = path.resolve(rootDir);
  const links = cases.flatMap((item) => [
    item.contextHref,
    ...(item.platforms || []).map((platform) => platform.contextHref),
  ]).filter(Boolean);
  for (const href of links) {
    const target = path.resolve(root, href);
    const relative = path.relative(root, target);
    if (relative.startsWith('..') || path.isAbsolute(relative)) {
      throw new Error(`REPORT_LINK_OUTSIDE_WORKSPACE: ${href}`);
    }
    if (!fs.existsSync(target) || !fs.statSync(target).isFile()) {
      throw new Error(`REPORT_LINK_TARGET_MISSING: ${href}`);
    }
  }
}
...
function readLatestExecutionReport(caseDir, options = {}) {
  ...
  return readExecutionReport(latest);
}
...
function writeCaseReports(caseDir, caseJson, state = {}, notes = [], report = null, options = {}) {
  ...
  const contextMarkdown = options.platform
    ? (currentReport ? renderCurrentContextMarkdown(reportCaseJson, latestReport) : renderContext(reportCaseJson, reportState, latestReport.result, latestReport.metrics, resolvedNotes, latestReport.events))
    : renderCaseOverviewMarkdown(caseDir, caseJson, resolvedNotes);
  const contextHtml = options.platform
    ? (currentReport ? renderCurrentContextHtml(reportCaseJson, latestReport) : renderContextHtml(reportCaseJson, reportState, latestReport.result, latestReport.metrics, resolvedNotes, latestReport.events, { runtimeDir, executionDir: latestReport.latest }))
    : renderCaseOverviewHtml(caseDir, caseJson, resolvedNotes);
  publishReportBundle(runtimeDir, { 'CONTEXT.md': contextMarkdown, 'CONTEXT.html': contextHtml }, {
    schemaVersion: 1,
    scope: options.platform ? 'platform-case' : 'case',
    platform: options.platform || null,
    executionId: latestReport.execution?.executionId || null,
    ...reportRendererInfo(),
  }, options);
  if (options.platform && !options.skipRootOverview) {
    publishReportBundle(caseDir, {
      'CONTEXT.md': renderCaseOverviewMarkdown(caseDir, caseJson, resolvedNotes),
      'CONTEXT.html': renderCaseOverviewHtml(caseDir, caseJson, resolvedNotes),
    }, { schemaVersion: 1, scope: 'case', platform: null, executionId: null, ...reportRendererInfo() }, options);
  }
  return {
...
function renderCaseOverviewHtml(caseDir, caseJson, notes = []) {
  ...
  const sourceText = fs.existsSync(sourcePath) ? readText(sourcePath) : '';
  const conversationNotes = notes.filter((note) => note.source === 'conversation');
  const platformCards = platforms.length
    ...
    : '<div class="not-run-message"><b>该用例尚未执行</b><span>执行后将在这里展示各平台结果和报告入口。</span></div>';
  const preconditionRows = (caseJson.preconditions || []).length
    ? caseJson.preconditions.map((item) => `<tr><td>${escapeHtml(item.id)}</td><td>${escapeHtml(item.text)}</td><td>${escapeHtml(displayPreconditionMode(item.checkMode))}</td></tr>`).join('\n')
    : '<tr><td colspan="3">无</td></tr>';
  ...
  const ruleRows = globalRules.length
    ...
    : '<tr><td colspan="4">无</td></tr>';
  const noteRows = conversationNotes.length
    ? conversationNotes.map((note) => `<tr>
        <td>${escapeHtml(formatDisplayTime(note.time))}</td>
        <td>${escapeHtml(note.appliesTo || '-')}</td>
        <td>${escapeHtml(note.stale ? '失效' : '生效')}</td>
        <td>${escapeHtml(note.text || '')}</td>
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>${escapeHtml(caseJson.identity.title)} - 用例详情</title>
  <style>
    :root { color-scheme: light; --bg:#f5fbff; --surface:#ffffff; --surface-soft:#f8fcff; --surface-tint:#eef8ff; --text:#111827; --muted:#5f6f86; --line:#d8e8f6; --accent:#0ea5e9; --accent-strong:#0284c7; --accent-soft:#e0f2fe; --pass:#059669; --pass-soft:#ecfdf5; --pass-line:#86efac; --fail:#dc2626; --fail-soft:#fef2f2; --fail-line:#fca5a5; --blocked:#b7791f; --blocked-soft:#fffbeb; --blocked-line:#fde68a; --unknown:#64748b; --unknown-soft:#f1f5f9; --shadow:0 12px 34px rgba(15, 55, 90, .08); }
    * { box-sizing: border-box; }
    body { margin: 0; background: linear-gradient(180deg, #eef8ff 0, var(--bg) 230px); color: var(--text); font: 14px/1.55 -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; }
    ...
    .not-run-message { display: grid; gap: 3px; padding: 18px; border: 1px dashed var(--line); border-radius: 6px; background: var(--surface-soft); color: var(--muted); }
    .not-run-message b { color: var(--text); font-size: 15px; }
    .source-rendered { padding: 2px 4px; overflow-wrap: anywhere; }
    .source-rendered > :first-child { margin-top: 0; }
    .source-rendered > :last-child { margin-bottom: 0; }
    .source-rendered h1, .source-rendered h2, .source-rendered h3, .source-rendered h4, .source-rendered h5, .source-rendered h6 { margin: 18px 0 8px; letter-spacing: 0; }
    .source-rendered h1 { font-size: 22px; }
    .source-rendered h2 { font-size: 18px; }
    .source-rendered h3 { font-size: 16px; }
    ...
    .source-rendered pre { max-height: 420px; padding: 12px; background: #f7f8fa; overflow: auto; white-space: pre-wrap; }
...
<main>
  ...
  <header>
    <div>
      <h1>${escapeHtml(caseJson.identity.title)}</h1>
      <div class="muted">用例详情 · 编号：${escapeHtml(caseJson.identity.caseNo || '-')} · ${escapeHtml(caseJson.identity.caseKey || '-')}</div>
    </div>
  ...
  <section class="source-section">
    <h2>原始用例</h2>
    <div class="source-rendered">${sourceText.trim() ? renderSourceMarkdown(sourceText) : '<p class="empty">原始用例内容不可用。</p>'}</div>
  </section>
  ...
  ${(caseJson.preconditions || []).length ? `<section>
    <h2>共享前置条件</h2>
    <div class="table-wrap"><table>
      <thead><tr><th style="width:100px">ID</th><th>内容</th><th style="width:140px">模式</th></tr></thead>
      <tbody>${preconditionRows}</tbody>
  ...
  ${conversationNotes.length ? `<section>
    <h2>用户补充</h2>
    <div class="table-wrap"><table>
...
function renderHistoricalIndexHtml(rootDir, cases = []) {
  ...
  const currentSummary = {
    ...
    inconclusive: currentRuns.filter((item) => item.verdict === 'INCONCLUSIVE').length,
  ...
  const currentMetrics = currentRuns.length ? `<section class="current-metrics" aria-label="当前执行指标">
    <div><span>直接证据</span><b>${escapeHtml(currentSummary.directEvidence)}</b></div><div><span>知识支持</span><b>${escapeHtml(currentSummary.knowledgeSupported)}</b></div><div><span>无法判断</span><b>${escapeHtml(currentSummary.inconclusive)}</b></div>
    <div><span>准备动作</span><b>${escapeHtml(currentSummary.preparationActions)}</b></div><div><span>暖状态复用</span><b>${escapeHtml(currentSummary.warmReuse)}</b></div><div><span>受控恢复</span><b>${escapeHtml(currentSummary.recoveries)}</b></div>
    <div><span>时限停止</span><b>${escapeHtml(currentSummary.timeLimitStops)}</b></div><div><span>计划修订</span><b>${escapeHtml(currentSummary.planRevisions)}</b></div><div><span>知识查询</span><b>${escapeHtml(currentSummary.knowledgeQueries)}</b></div>
  </section>` : '';
    ...
    ? cases.map((item, index) => {
      ...
      const platformDetails = platforms.length
        ? `<details class="platform-details">
          ...
          const progressText = platform.schemaFamily === 'current' ? (platform.verdictBasis || '-') : (platform.stepsSummary || '-');
          return `<span class="platform-brief ${escapeHtml(platformStatusClass)}">
            <span class="platform-brief-head"><b>${escapeHtml(displayPlatform(platform.platform))}</b><em>${escapeHtml(displayStatus(platform.status))}</em><small>${escapeHtml(progressText)} · ${escapeHtml(platformDuration)}</small></span>
          </span>`;
        }).join('')}</div>
          ...
          if (platform.status === 'FAIL') {
            ...
            platformResult = platform.schemaFamily === 'current' ? (platform.reason || '要求已满足') : '全部步骤通过';
          }
          const reasonText = platform.reason || platformConclusion(platform);
          ...
          return `<article class="platform-detail-card ${escapeHtml(platformStatusClass)}">
            ...
            <div class="platform-detail-meta">
              <div><span>${platform.schemaFamily === 'current' ? '依据' : '步骤'}</span><b>${escapeHtml(platform.schemaFamily === 'current' ? (platform.verdictBasis || '-') : (platform.stepsSummary || '-'))}</b></div>
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .current-metrics { display:grid;grid-template-columns:repeat(9,minmax(0,1fr));gap:8px;margin:0 0 18px;padding:12px; }
    .current-metrics div{min-width:0;padding:8px;border:1px solid var(--line);border-radius:6px;background:var(--surface-soft)}
    .current-metrics span{display:block;color:var(--muted);font-size:11px;white-space:nowrap}.current-metrics b{display:block;margin-top:2px;font-size:18px}
    ...
    @media (max-width: 1060px) { .current-metrics{grid-template-columns:repeat(3,minmax(0,1fr))}.platform-status-grid { grid-template-columns: repeat(3, minmax(0, 1fr)); } .platform-detail-meta { grid-template-columns: 76px 92px minmax(220px, 1fr) minmax(220px, 1fr); } }
