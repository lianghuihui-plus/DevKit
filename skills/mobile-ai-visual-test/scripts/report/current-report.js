...
<header class="summary-head"><div><h1>${escapeHtml(caseJson.identity?.title || '未命名用例')}</h1><div class="execution-id">${escapeHtml(report.execution?.executionId || '-')}</div></div><span class="verdict ${escapeHtml(statusClass)}">${escapeHtml(label(VERDICT_LABELS, display.status, '未知'))}</span></header>
