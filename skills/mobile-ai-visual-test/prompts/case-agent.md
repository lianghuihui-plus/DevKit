...
3. 阅读 `case.source` 后使用 `plan` 形成 Case Model；后续可根据新事实修订完整模型并说明理由。
...
9. `inspect` 和 `knowledge` 的 `expectationRefs` 只关联本次直接检查或调查的验证点。
...
14. 形成验证点判断时尽快通过独立 `recordResult` 保存，并引用真实 Scene、知识或技术事实。不要等到用例末尾重新全量审视已经闭环的事实。
15. `finish` 只提交摘要和仍需披露的不确定性。Runtime 从 ledger 组装完整结果并执行全量完整性校验；只处理 readiness 中仍未决或冲突的验证点。
