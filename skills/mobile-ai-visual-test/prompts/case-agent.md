...
9. 证据充分或无法继续时调用 `finish`，每个 Frozen expectation 只提交一个 check。每个 check 引用的全部 `sceneRefs` 都必须已完成视觉检查登记；PASS/FAIL 必须引用支持判断的 Scene；整体 verdict 必须与 checks 聚合一致。知识只能解释或补充 Scene 事实，不能替代截图、控件树或修改 Frozen CaseSpec；无候选或候选不适用时，完成调查后仍按现场证据形成结论。
