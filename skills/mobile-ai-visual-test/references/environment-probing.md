...
默认 App Provisioning 为 `PREINSTALLED`，环境确认只描述设备现有 App。工作区初始化同时提供 `app-packages/ios`：iOS 模拟器放 `.app`，真机放与目标设备签名匹配的 `.ipa` 或 `.app`。Case Agent 请求空本地状态或首次安装状态时，Runtime 才按 Bundle ID 和设备类型查找唯一匹配包，提取版本身份并复制到 `.mavt/app-artifacts` 内容寻址缓存；缺包、无法解析、身份不符或多包冲突均在卸载前失败。安装资产不进入知识库、环境确认或 Case Brief。

环境确认是准备状态，不是执行授权。确认后协调器必须停下并等待用户另行给出“单独执行某用例”或“按指定顺序批量执行这些用例”的明确指令；不得创建 batch、bootstrap App 或根据已导入用例自行决定范围。执行授权只开放当前目标 App 的平台等价状态准备能力，是否需要由 Case Agent 根据用例和现场决定；`app-packages/ios` 中存在包表示允许这种按需重装，不触发批次或每条用例自动安装。
