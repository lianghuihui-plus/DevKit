...
{"type":"inputText","text":"13800000000","mode":"replace","target":"当前已聚焦输入框"}
{"type":"inputText","x":360,"y":640,"text":"13800000000","mode":"replace","target":"手机号输入框","coordinateSource":"layout","coordinateEvidence":"控件树存在手机号输入框 bounds"}
...
`inputText.mode` 只支持 `replace` 和 `append`，缺省统一归一为 `replace`。人工步骤明确包含“追加输入、继续输入、在末尾输入”等语义时才使用 `append`；“输入、填写、清空后输入、修改为、替换为”均使用 `replace`。对于 `goal=input_text` 的业务步骤，Case Engine 会冻结 `inputMode` 和目标文本，Agent 不得改变这两个值。
...
| `global-rule` | `tap`、`toggle`、`longPress`、`back`、`home`、`wait` |
...
业务步骤和全局规则只接受 `layout`、`visual`、`pixel`；前置条件 Flow 额外接受 `flow`；`manual` 在所有正式执行中禁用。Agent 常见表达会在校验前归一并只保存规范值：`screenshot/image -> visual`，`uiTree -> layout`。DecisionRequest 中的 `actionConstraints` 是当前平台和作用域的机器可读权威约束。
...
| 平台 | 差异 |
| --- | --- |
| Android | `inputText` 不接受 `x/y`；必须先 `tap` 聚焦；ASCII 与非 ASCII 都由 MAVT Input IME 按 mode 写入，避免两条路径语义不一致 |
| HarmonyOS | `inputText` 需要 `x/y/text`；`replace` 在单个 atom 内执行聚焦、全选、删除、输入，并在有限次数内等待布局值稳定；首个旧值不会立即判失败 |
| iOS | `inputText` 不接受 `x/y`；`replace` 通过 WDA clear 后写值；可读的非安全输入框执行 `append` 时先读取当前值、合成最终值、clear 后一次写入并校验；多输入框页面必须先 `tap` 聚焦 |
...
atoms 只做最小能力：`tap`、`long-press`、`swipe`、`input-text`、`screenshot`、`dump-tree`、`foreground`、`logs`、`launch-app`、`restart-app`、`wait`。`input-text replace` 内部的清空与写入是一个不可分割的平台原子动作，不属于跨业务步骤编排。
