...
框架产物统一使用 `deviceId` 表示设备身份。环境确认入口兼容只提供旧字段 `device` 的历史调用，并在写入前转换为 `deviceId`；同时提供两个字段但值不一致时直接拒绝，防止设备身份含糊。规范化后的 confirmation、execution request、batch contract 和 execution snapshot 不再保留 `device`。读取历史冻结 batch/execution 时也在进入设备会话前完成同样转换，但不改写原历史文件。平台脚本的命令行参数仍为 `--device`，该名称只属于 adapter 边界，不进入框架数据契约。
