...
现场事实能证明发生了什么，不能单独证明业务定性。知识调查由结论所需信息触发，而不是由结论正负机械触发。出现以下情况时查询：

- 结论依赖 Scene 之外的产品规则、平台或版本差异、账号、配置或其他外部条件。
- 预期与现场不一致，且平台、版本、账号、配置或其他 Scene 外规则可能改变 PASS/FAIL、豁免或不适用定性；此时必须在 `recordResult` 前调用 `knowledge`。
- 截图和控件树无法独立解释异常，并且知识可能补充缺失的业务语义。
- 无法确定下一步是因为缺少已知业务约束，而不是因为尚未观察或检查现有证据。

实际结果不符、操作失败或准备形成 `FAIL`、`INCONCLUSIVE`、`BLOCKED`，本身都不机械触发查询；只有确认不存在上述外部规则依赖时才可直接按现场证据定性。查询无候选或候选均不适用时仍按现场证据判断。
