...
现场事实能证明发生了什么，不能单独证明业务定性。知识调查由下一步所需信息触发，不由结论正负机械触发：

- 预期不符、操作无效果或异常反复，且平台、版本、账号、配置、条件适用性、同类异常处理或原文歧义可能影响下一步时，尽早调用 `knowledge`，不要等到 `recordResult` 或 `finish` 收口。
- 结论依赖 Scene 之外的产品规则或其他外部条件。
- 截图和控件树无法独立解释异常，并且知识可能补充缺失的业务语义。
- 无法确定下一步是因为缺少已知业务约束，而不是因为尚未观察或检查现有证据。

不存在上述外部依赖时不机械查询。无候选或候选均不适用时，仍按现场证据判断。
