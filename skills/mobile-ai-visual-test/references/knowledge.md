...
正常、无疑问且不依赖 Scene 外信息的 PASS 可以不查询。以下情况必须调查：

- 实际结果与预期不符。
- 截图和控件树无法独立解释当前现象。
- 操作失败、重复尝试无进展，或无法确定下一步。
- 结论可能受平台、版本、账号、配置或其他外部条件影响。
- 准备形成 FAIL 或 INCONCLUSIVE 等负向结论。

查询无候选或候选均不适用时，完成调查后仍按现场证据形成结论。瞬态加载经重新观察恢复且不影响结论时不需要查询。
