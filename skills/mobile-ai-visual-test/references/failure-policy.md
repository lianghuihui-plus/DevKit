...
每次动作自动采集新 Scene，并返回请求坐标、投递坐标、可选设备触点、落点标注图和整屏像素比较。框架不判断动作是否命中业务目标。动作无效果、证据冲突或落点可疑时，先检查标注图；现场可能变化时重新 `observe`，再决定纠正动作或结论。动作已发出但结果未知时记录 `actionOutcomeUnknown`，先观察现场，不自动重放；旧 Scene 动作返回 `SCENE_CHANGED`。
