...
业务判断与结果语义统一见[用例执行原则](case-execution-principles.md)。本文只定义技术失败、恢复和证据安全边界。
...
每次普通动作自动采集新 Scene，并返回请求坐标、投递坐标、确定性技术执行/核验结果、可选设备触点、落点标注图以及前后 Scene/截图引用。Runtime 不通过截图 hash 判断画面是否变化，也不判断动作是否命中业务目标；截图 hash 只用于产物完整性。技术结果已知时，Agent 根据动作是否可安全重放、现场是否仍适用以及能否获得新信息，自主选择重试、换动作、换路径或停止。动作无效果、证据冲突或落点可疑时按需检查前后截图和标注图。现场可能变化时重新 `observe`。动作已发出但结果未知时记录 `actionOutcomeUnknown`，先观察现场，禁止自动重放；旧 Scene 动作返回 `SCENE_CHANGED`。

`ACTION_EFFECT_MISMATCH` 表示动作已经执行但确定性核验不符合请求，属于技术动作失败。它本身不能支撑产品 FAIL；Agent 读取当前 Scene 和技术证据后，自主选择安全且有信息增益的恢复。明确技术失败的恢复与 BLOCKED 收口不要求额外知识调查。
...
登录账号、验证码和其他业务凭据由测试环境在执行前预置，框架不猜测、生成或轮试账号，也不把凭据写入 execution、事件或报告。缺少可用账号本身不直接形成 `BLOCKED`：尚未进入相应 CHECK 验证且必要条件无法在授权范围内建立时，按执行原则判断 `NOT_RUN`；已进入验证但证据不足时使用 `INCONCLUSIVE`；只有有效技术事实直接阻止当前 CHECK 时才使用 `BLOCKED` 并引用该事实。
