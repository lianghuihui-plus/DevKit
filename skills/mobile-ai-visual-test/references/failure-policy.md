...
executionStatus 表达 `COMPLETED`、`STOPPED_BY_BUDGET`、`TECHNICALLY_BLOCKED` 等执行完整性，不替代 verdict。达到 30 分钟只停止新设备调用；若已有充分且最新的证据，仍可形成 PASS 或 FAIL。若最新状态变化后没有取得可用观察，只能形成带 `observationUnavailable` 和明确证据缺口的 `INCONCLUSIVE + STOPPED_BY_BUDGET`；变化前的旧观察不能作为当前观察引用。
