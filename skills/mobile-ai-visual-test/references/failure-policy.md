...
每次普通动作自动采集新 Scene，并返回请求坐标、投递坐标、确定性技术执行/核验结果、可选设备触点、落点标注图以及前后 Scene/截图引用。Runtime 不通过截图 hash 判断画面是否变化，也不判断动作是否命中业务目标；截图 hash 只用于产物完整性。技术结果已知、原动作可安全重放且现场仍适用时，Agent 优先进行一次有限重试；动作无效果、证据冲突或落点可疑时，由 Agent 按需打开前后截图和标注图判断。现场可能变化时重新 `observe`，再决定纠正动作或结论。动作已发出但结果未知时记录 `actionOutcomeUnknown`，先观察现场，禁止自动重放；旧 Scene 动作返回 `SCENE_CHANGED`。

`ACTION_EFFECT_MISMATCH` 表示动作已经执行但确定性核验不符合请求，属于技术动作失败。它本身不能支撑产品 FAIL；对 `replace` 输入等幂等动作，Agent 确认当前目标后可有限重试一次，仍失败且直接阻止检查点时形成 BLOCKED。明确技术失败的恢复与 BLOCKED 收口不要求额外知识调查。
