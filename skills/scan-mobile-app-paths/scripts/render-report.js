...
const { parseArgs, required, resolveScanDir, loadScan, loadGraph, loadFrontier, readJson, writeTextAtomic, contextDir, output, main } = require('./lib/common');
...
main(() => {
  ...
  for (const contextId of runContextIds(scan)) {
    ...
    const metrics = contextMetrics(graph, frontier, runtime); metricsByContext[contextId] = metrics;
    ...
    stabilityRows.push(`| ${contextId} | ${metrics.observations} | ${metrics.observationSamples} | ${metrics.averageSamplesPerObservation === null ? '-' : metrics.averageSamplesPerObservation.toFixed(1)} | ${(metrics.observationStabilityWaitMs / 1000).toFixed(1)} 秒 | ${metrics.visualVarianceObservations} | ${metrics.noStateChangeActions} |`);
