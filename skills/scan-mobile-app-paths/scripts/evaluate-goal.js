...
main(() => {
  ...
  if (command === 'evaluate') {
    ...
    result.status = 'AWAITING_HUMAN_CONFIRMATION'; result.matchedVisualStateId = decision.visualStateId; result.matchedReachableStateId = decision.reachableStateId; result.evidenceObservationId = observationId; result.candidateDecisionIds.push(decisionId); result.decisions.push(decision);
    if (args.frontierId) { const frontier = loadFrontier(scanDir, goal.contextId); const item = frontier.items.find(x => x.id === args.frontierId); if (!item || item.status !== 'EXPLORED') fail('Goal candidate frontier must already be committed or otherwise explored', 'GOAL_FRONTIER_INVALID'); decision.frontierId = item.id; }
    transitionWithOps(scanDir, 'PAUSED', 'GOAL_CANDIDATE_REVIEW', 'goalConfirmationRequested', { goalId: goal.goalId, decisionId, status: match.status, observationId }, [{ path: 'goal/match-result.json', op: 'REPLACE', value: result }]);
  ...
  if (command === 'update-policy') {
    ...
    goal.resultPolicy.maxVerifiedPaths = next; commitEvent(scanDir, 'goalPolicyUpdated', { goalId: goal.goalId, maxVerifiedPaths: next }, [{ path: 'goal/goal.json', op: 'REPLACE', value: goal }]);
  ...
  if (command === 'continue-search') {
    ...
    result.status = 'SEARCHING'; transitionWithOps(scanDir, 'SCANNING', null, 'goalSearchContinued', { goalId: goal.goalId }, [{ path: 'goal/match-result.json', op: 'REPLACE', value: result }]);
