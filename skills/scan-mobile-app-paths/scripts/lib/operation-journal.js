...
const { nextId, now, commitEvent, readJson } = require('./common');
...
function finishDeviceOperation(scanDir, operation, status, { evidenceRef = null, reasonCode = null } = {}) {
  ...
  if (status === 'UNKNOWN_OUTCOME') {
    ...
    const scan = readJson(path.join(scanDir, 'scan.json')); if (!['COMPLETED', 'PARTIAL', 'BLOCKED', 'FAILED'].includes(scan.status)) { scan.status = 'PAUSED'; scan.reasonCode = 'OPERATION_OUTCOME_UNKNOWN'; scan.pausedAt = now(); scan.updatedAt = scan.pausedAt; ops.push({ path: 'scan.json', op: 'REPLACE', value: scan }); }
  }
...
function recoverDeviceOperations(scanDir) {
  const dir = path.join(scanDir, 'operations'); if (!fs.existsSync(dir)) return 0; const scan = readJson(path.join(scanDir, 'scan.json')); let recovered = 0;
  for (const name of fs.readdirSync(dir).filter(item => item.endsWith('.json'))) {
    ...
    if (!['COMPLETED', 'PARTIAL', 'BLOCKED', 'FAILED'].includes(scan.status)) { scan.status = 'PAUSED'; scan.reasonCode = 'OPERATION_OUTCOME_UNKNOWN'; scan.pausedAt = now(); scan.updatedAt = scan.pausedAt; ops.push({ path: 'scan.json', op: 'REPLACE', value: scan }); }
    const store = require('./event-store'); const appended = store.append(scanDir, { type: 'deviceOperationOutcomeUnknown', at: now(), scanId: scan.scanId, contextId: operation.contextId, operation, projectionOps: ops }); const appliedOps = appended.record.projectionOps || []; store.applyProjectionOps(scanDir, appliedOps); store.markApplied(scanDir, appended.head, appliedOps); recovered += 1;
  }
  return recovered;
}
