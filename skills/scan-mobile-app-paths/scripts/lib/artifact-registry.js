...
const PROJECTION_PATTERNS = [
  ...
  /^plan\.json$/,
  /^continuation\.json$/,
  /^contexts\/[^/]+\/(context|graph|frontier|frontier-suggestions|metrics|live-cursor|exploration-start|verification-queue|back-capabilities|visual-equivalence|state-equivalence)\.json$/,
  /^known\/contexts\/[^/]+\.json$/,
  /^attempts\/[^/]+\.json$/,
  /^operations\/[^/]+\.json$/,
