...
function nextWork({ scanDir, scan, contextId, graph, frontier, metrics }) {
  const queue = loadVerificationQueue(scanDir, contextId); const failed = queue.items.filter(item => item.status === 'FAILED'); const retryableFailed = failed.filter(item => Number(item.attemptCount || 0) < MAX_VERIFICATION_ATTEMPTS); const terminalFailed = failed.filter(item => Number(item.attemptCount || 0) >= MAX_VERIFICATION_ATTEMPTS); const running = queue.items.filter(item => item.status === 'RUNNING'); const pending = [...queue.items.filter(item => item.status === 'PENDING'), ...retryableFailed];
