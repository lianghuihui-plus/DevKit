...
function buildContext(appMapRoot, contextId, graph) {
  ...
  const reachableEdges = graph.edges.filter(isRunnableEdge).map(edge => {
    const sourceRunId = edge.evidence?.sourceRunId || edge.provenance?.[0]?.runId; const before = sourceRunId && edge.evidence?.beforeObservationId ? evidenceFor({ runId: sourceRunId, observationId: edge.evidence.beforeObservationId }, { embedScreenshot: true }) : null; const after = sourceRunId && edge.evidence?.afterObservationId ? evidenceFor({ runId: sourceRunId, observationId: edge.evidence.afterObservationId }, { embedScreenshot: true }) : null; const rawOutcome = sourceRunId && edge.evidence?.rawOutcomeObservationId ? evidenceFor({ runId: sourceRunId, observationId: edge.evidence.rawOutcomeObservationId }, { embedScreenshot: true }) : null;
