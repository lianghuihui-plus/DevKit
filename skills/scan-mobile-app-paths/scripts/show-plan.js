...
main(() => {
  ...
  output({ schemaVersion: 1, ok: true, deprecated: true, replacement: 'preview-plan.js + init-scan.js --confirmed-plan-hash', planHash: planHash(plan), plan, confirmationPrompt: '请在同一确认环节查看全部预设配置、当前选择、预算覆盖、当前 context 的预算和执行上限；可确认当前计划，也可改选适用预设或自定义预算。确认后才能开始扫描。' });
});
