...
const { normalizeDeviceType } = require('./lib/device-detection');
...
main(() => {
  ...
  const target = validateTarget({ ...old, bundleName: args.bundleName || old.bundleName, entryAbility: args.entryAbility || old.entryAbility, environment: args.environment || old.environment, deviceId: args.device || old.deviceId, deviceType: args.deviceType ? normalizeDeviceType(args.deviceType) : old.deviceType ?? null, appVersion: args.appVersion ?? old.appVersion, buildVersion: args.buildVersion ?? old.buildVersion });
  ...
  commitEvent(scanDir, 'targetConfirmed', { target: { ...target, deviceId: target.deviceId, deviceType: target.deviceType || null } }, [{ path: 'target.json', op: 'REPLACE', value: target }, { path: 'scan.json', op: 'REPLACE', value: scan }]);
