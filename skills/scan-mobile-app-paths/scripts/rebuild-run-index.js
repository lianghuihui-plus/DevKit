...
main(() => {
  ...
  for (const name of fs.readdirSync(runsDir, { withFileTypes: true }).filter(x => x.isDirectory()).map(x => x.name).sort()) {
    try {
      ...
      validate(scanDir, scan.status); const target = readJson(path.join(scanDir, 'target.json')); const contextIds = runContextIds(scan); const graphs = contextIds.map(contextId => readJson(path.join(scanDir, 'contexts', contextId, 'graph.json'), emptyGraph(contextId)));
      runs.push({ scanId: scan.scanId, parentScanId: scan.parentScanId || null, mapRevisionId: scan.mapRevisionId || scan.scanId, mapBaseRevisionId: scan.mapBaseRevisionId || null, relativePath: `runs/${scan.scanId}`, appVersion: target.appVersion || null, buildVersion: target.buildVersion || null, versionKey: versionKey(target), scanMode: scan.scanMode, scanScope: scan.scanScope, contexts: contextIds, status: scan.status, startedAt: scan.startedAt || scan.createdAt, finalizedAt: scan.finalizedAt, visualStateCount: graphs.reduce((n, g) => n + (g.visualStates || []).length, 0), reachableStateCount: graphs.reduce((n, g) => n + (g.reachableStates || []).length, 0), edgeCount: graphs.reduce((n, g) => n + (g.edges || []).length, 0) });
