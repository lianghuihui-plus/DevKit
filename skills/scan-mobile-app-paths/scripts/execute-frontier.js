...
main(() => {
  ...
  if (command === 'review-restore') {
    ...
    const visualReviewId = safeSegment(required(args, 'visualReviewId'), 'visualReviewId'); const visualReview = assertAcceptedVisualReview(scanDir, { visualReviewId, contextId, observationId, reviewType: 'RESTORE_STATE' }); const popupAssessment = assertInterruptionCleanupReview(visualReview, disposition);
    const dismissAction = jsonArg(required(args, 'dismissAction'), null, 'dismissAction JSON'); assertDismissalMatchesAssessment(popupAssessment, dismissAction); const dismissal = runJson('popup-dismiss-runner.js', ['--scan-dir', scanDir, '--context', contextId, '--observation-id', observationId, '--owner-type', 'ATTEMPT', '--owner-id', attemptId, '--visual-review-id', visualReviewId, '--action', JSON.stringify(dismissAction)]).actionResult;
    ...
    attempt.interruptions.push({ schemaVersion: 2, phase: 'RESTORE', restoreId, popupKind: popupAssessment.popupKind, popupClass: disposition, graphRole: popupAssessment.graphRole, businessRelevance: popupAssessment.businessRelevance || 'NON_BUSINESS', popupAssessment, visualReviewId, beforeObservationId: observationId, dismissalActionResultId: dismissal.actionId, afterObservationId: after.observation.observationId, handledAt: now() }); attempt.updatedAt = now(); commitAttempt(scanDir, 'attemptPopupDismissed', { contextId, attemptId, phase: 'RESTORE', restoreId, popupClass: disposition, popupKind: popupAssessment.popupKind, visualReviewId, beforeObservationId: observationId, dismissalActionResultId: dismissal.actionId, afterObservationId: after.observation.observationId, dismissalCount: attempt.interruptions.filter(item => item.phase === 'RESTORE').length }, attempt); return performRestore(attempt, { restoreId, observationId: after.observation.observationId });
  }
  if (command === 'act') {
    ...
    try {
      ...
      current.afterObservationId = after.observation.observationId; current.rawOutcomeObservationId ||= after.observation.observationId; current.stableOutcomeObservationId = null; current.reviewObservationId = after.observation.observationId; current.sourceComparison = compareObservations(scanDir, current.beforeObservationId, current.afterObservationId); current.status = 'AWAITING_VISUAL_REVIEW'; current.visualReviewId = null; current.visualReviewStatus = null; current.updatedAt = now(); commitAttempt(scanDir, 'visualReviewRequested', { contextId, attemptId, phase: 'OUTCOME', observationId: current.reviewObservationId, reviewType: 'PAGE_OUTCOME', sourceComparison: current.sourceComparison }, current); return attemptOutput({ schemaVersion: 1, ok: true, attempt: current, visualReviewRequest: { reviewType: 'PAGE_OUTCOME', ...buildReviewRequest(scanDir, current, 'OUTCOME') } });
  ...
  if (command === 'review-outcome') {
    ...
    const visualReview = assertAcceptedVisualReview(scanDir, { visualReviewId: attempt.visualReviewId, contextId, observationId, reviewType: 'PAGE_OUTCOME' });
    ...
    if (disposition === 'NO_STATE_CHANGE') {
      ...
      attempt.status = 'NO_STATE_CHANGE'; attempt.outcomeKind = null; attempt.reviewObservationId = null; attempt.sourceComparison = comparison; attempt.stableOutcomeObservationId = observationId; attempt.noStateChange = { beforeObservationId: attempt.beforeObservationId, afterObservationId: observationId, comparison, confirmedAt: now() }; attempt.updatedAt = now();
    ...
    if (disposition === 'TRANSIENT') {
      ...
      attempt.stabilityChecks.push({ phase: 'OUTCOME', beforeObservationId: observationId, afterObservationId: after.observation.observationId, checkedAt: now() }); attempt.afterObservationId = after.observation.observationId; attempt.stableOutcomeObservationId = null; attempt.reviewObservationId = after.observation.observationId; attempt.sourceComparison = compareObservations(scanDir, attempt.beforeObservationId, attempt.afterObservationId); attempt.status = 'AWAITING_VISUAL_REVIEW'; attempt.visualReviewId = null; attempt.visualReviewStatus = null; attempt.updatedAt = now(); commitAttempt(scanDir, 'visualReviewRequested', { contextId, attemptId, phase: 'OUTCOME', beforeObservationId: observationId, observationId: after.observation.observationId, reviewType: 'PAGE_OUTCOME', checkCount: attempt.stabilityChecks.length, sourceComparison: attempt.sourceComparison }, attempt); return attemptOutput({ schemaVersion: 1, ok: true, attempt, visualReviewRequest: { reviewType: 'PAGE_OUTCOME', ...buildReviewRequest(scanDir, attempt, 'OUTCOME') } });
    }
    if (NON_GRAPH_POPUP_DISPOSITIONS.includes(disposition)) {
      if ((attempt.interruptions || []).length >= 3) fail('Attempt popup dismissal limit reached', 'POPUP_DISMISS_LIMIT'); const popupAssessment = assertInterruptionCleanupReview(visualReview, disposition); const dismissAction = jsonArg(required(args, 'dismissAction'), null, 'dismissAction JSON');
      assertDismissalMatchesAssessment(popupAssessment, dismissAction);
      const dismissal = runJson('popup-dismiss-runner.js', ['--scan-dir', scanDir, '--context', contextId, '--observation-id', observationId, '--owner-type', 'ATTEMPT', '--owner-id', attemptId, '--visual-review-id', visualReview.visualReviewId, '--action', JSON.stringify(dismissAction)]).actionResult;
      ...
      const interruption = { schemaVersion: 2, phase: 'OUTCOME', popupKind: popupAssessment.popupKind, popupClass: disposition, graphRole: popupAssessment.graphRole, businessRelevance: popupAssessment.businessRelevance || 'NON_BUSINESS', popupAssessment, visualReviewId: visualReview.visualReviewId, beforeObservationId: observationId, dismissalActionResultId: dismissal.actionId, afterObservationId: after.observation.observationId, handledAt: now() };
      attempt.interruptions.push(interruption); attempt.afterCleanupObservationId = after.observation.observationId; attempt.afterObservationId = after.observation.observationId; attempt.stableOutcomeObservationId = after.observation.observationId; attempt.updatedAt = now();
      ...
      interruption.cleanupComparison = cleanupComparison;
    ...
    if (disposition === 'BUSINESS_MODAL') {
      ...
      attempt.popupAssessment = assertBusinessModalReview(visualReview);
    }
    ...
    attempt.stableOutcomeObservationId = observationId; attempt.outcomeKind = disposition === 'BUSINESS_MODAL' ? 'modal' : 'full-screen'; attempt.status = 'READY_TO_COMMIT'; attempt.reviewObservationId = null; attempt.updatedAt = now(); commitAttempt(scanDir, 'attemptReadyToCommit', { contextId, attemptId, frontierId: attempt.frontierId, observationId, disposition, beforeObservationId: attempt.beforeObservationId, actionResultId: attempt.actionResultId, rawOutcomeObservationId: attempt.rawOutcomeObservationId, stableOutcomeObservationId: attempt.stableOutcomeObservationId, afterObservationId: attempt.afterObservationId, outcomeKind: attempt.outcomeKind, popupAssessment: attempt.popupAssessment || null }, attempt); return attemptOutput({ schemaVersion: 1, ok: true, attempt });
  }
