...
执行扫描时完整读取 `references/workflow.md`。导出前置条件 Flow 时读取 `references/precondition-flow-export.md`。修改或排查引擎时再读取 `references/architecture.md` 与 `references/artifact-contract.md`。
...
- 产物按 `Projection / Evidence / Canonical / Generated` 分类：Run 内状态型 JSON 必须可由 timeline 重建；Observation、ActionResult、VerificationExecution 与日志作为本 Run 证据被引用校验；`maps/<context>` 是可持续扩展的 canonical map；报告、Snapshot 与 Dashboard 为可重建生成物。
...
- `references/precondition-flow-export.md`：从当前 Snapshot 导出前置条件 Flow 的命令、校验和交付规则。
